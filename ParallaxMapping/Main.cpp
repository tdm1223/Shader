...
// D3D 관련
std::unique_ptr<ParallaxMappingManager> gManager;
...
{
    // 프로그램 이름
    std::string appName = "Parallax Mapping";

    gManager = std::make_unique<ParallaxMappingManager>();
    ...
    if (!gManager->Init(hWnd, "normalMapping.fx", "SphereWithTangent.x", "Fieldstone_DM.tga"))
    {
        PostQuitMessage(1);
    }
    ...
    while (msg.message != WM_QUIT)
    {
        ...
        else // 메시지가 없으면 업데이트하고 장면을 그린다
        {
            ...
            {
                gManager->rotationY -= 0.4f * Util::PI / 180.0f;
                if (gManager->rotationY > 2 * Util::PI)
                {
                    gManager->rotationY -= 2 * Util::PI;
                }
            }
            ...
            {
                gManager->worldCameraPosition.z += 1.0f;
            }
