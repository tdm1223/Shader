...
// D3D 관련
std::unique_ptr<ToonShaderManager> gManager;
...
{
    // 프로그램 이름
    std::string appName = "Toon Shader";

    gManager = std::make_unique<ToonShaderManager>();
    ...
    if (!gManager->Init(hWnd, "ToonShader.fx", "teapot.x", ""))
    {
        PostQuitMessage(1);
    }
    ...
    while (msg.message != WM_QUIT)
    {
        ...
        {
            gManager->RenderFrame(Util::string_format("ESC: 데모종료\nToonShaderParam : %.1f (A,D로 Toon Shader 조절)", gManager->toonShaderParam));
