...
// D3D 관련
std::unique_ptr<ColorConversionManager> gManager;
...
void ProcessInput(HWND hWnd, WPARAM keyPress) noexcept
{
    ...
    case '2':
    case '3':
        gManager->postProcessIndex = keyPress - '0' - 1;
        break;
    default:
        break;
    }
...
{
    // 프로그램 이름
    std::string appName = "Color Conversion";

    gManager = std::make_unique<ColorConversionManager>();
    ...
    if (!gManager->Init(hWnd, "EnvironmentMapping.fx", "TeapotWithTangent.x", "Fieldstone_DM.tga"))
    {
        PostQuitMessage(1);
    }
