#include "ColorConversionManager.h"

ColorConversionManager::ColorConversionManager()
{
    rotationY = 0.0f;
...
bool ColorConversionManager::Init(HWND hWnd, std::string shaderName /*= ""*/, std::string modelName /*= ""*/, std::string textureName /*= ""*/)
{
    // D3D를 초기화
    if (!InitD3D(hWnd))
    {
    ...
    // 렌더타깃을 만든다.
    if (FAILED(d3dDevice->CreateTexture(Util::WIN_WIDTH, Util::WIN_HEIGHT,
...
void ColorConversionManager::RenderScene()
{
#pragma region 1. 장면을 렌더타깃 안에 그린다
    // 현재 하드웨어 벡버퍼
...
void ColorConversionManager::InitFullScreenQuad() noexcept
{
    // 정점 선언을 만든다
...
bool ColorConversionManager::LoadAssets(std::string shaderName /*= ""*/, std::string modelName /*= ""*/, std::string textureName /*= ""*/)
{
    // DiffuseMap 로딩
